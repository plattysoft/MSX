...
FILE "raftoid.sc2.plet5"

10 color 2,1,1
20 SCREEN 2,2,0
30 CMD WRTVRAM 1, 0
40 DEFINT A-Z
41 DEFSNG AX,X,VX,VY,BY,BX,RT,PR
...
401 IX=INT(BX):IY=INT(BY)
410 if vx>0 THEN sa=(IX+6-8)\16 ELSE sa=(IX-1-8)\16
411 sb=(IY+3)\8
412 v=VPEEK(&H1800+sa*2+1+sb*32)
413 if v>9 AND v<48 THEN vx=-vx:GOSUB 650
415 if vy>0 THEN sb=(IY+6)\8 ELSE sb=(IY-1)\8
416 sa=(IX+3-8)\16
...
686 if pb=8 THEN IF sb=(IY+3)\8 THEN vx=-vx ELSE vy=-vy
687 if sb MOD 2 = 1 THEN nc=97+(l MOD 3)*2:cn=96+(l MOD 3)*2 ELSE nc=96+(l MOD 3)*2:cn=97+(l MOD 3)*2
...
840   NEXT I
841 NEXT J
875 bx=102:by=172:IX=102:IY=172:vx=0.5:vy=-2
876 x=86:ax=0
...
4009 next i
4011 T$="PRESS FIRE TO START":TX=5:TY=14:GOSUB 9090
...
4054   TIME=0
4055   T$ = "                    ":GOSUB 9090
4056   IF TIME<10 GOTO 4055 ELSE TIME=0
4057   T$ = "PRESS FIRE TO START":GOSUB 9090
4058   IF TIME<10 GOTO 4057 ELSE TIME=0
4059 next k
4080 GOSUB 800
4090 GOTO 100
