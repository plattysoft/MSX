...
9120 TT = &H1800+(X\8)+((Y+32)\8)*32
9121 T0 = VPEEK (TT)
9122 T1 = VPEEK (TT+1)
9123 T2 = VPEEK (TT+2)
9124 IF D=0 THEN T3=VPEEK(TT-64+2):T4=VPEEK(TT-32+2)
9125 IF D=11 THEN T3=VPEEK(TT-64):T4=VPEEK(TT-32)
9130 VPOKE &H1AE8,T0
9131 VPOKE &H1AE9,T1
9132 VPOKE &H1AEA,T2
9134 VPOKE &H1AAA,T3
9135 VPOKE &H1ACA,T4
9140 IF T0<64 AND T2<64 THEN GS=3
9141 IF T3>=64 AND T4>=64 THEN IF D=0 THEN X=X-3 ELSE X=X+3
9190 RETURN
