...
9101 S=STICK(0)
9102 VX=0
9103 IF S=3 THEN VX=1
9104 IF S=7 THEN VX=-1
9105 IF STRIG(0) THEN GS=2:VY=-13:RETURN
9106 X=X+VX
9112 IF S=3 THEN IF D=11 OR SA=4 THEN D=0:AD=1:SA=0:ST=0 ELSE GOTO 9118
9113 IF S=7 THEN IF D=0 OR SA=4 THEN D=11:AD=1:SA=0:ST=0 ELSE GOTO 9118
9114 IF S=0 THEN SA=4:ST=3:YO=0' SA=4 marks a resting position
9115 IF S=5 THEN YO=3:ST=4:SA=4
9117 GOTO 9140
9118 IF S0=2 THEN S0=0 ELSE S0=S0+1:GOTO 9140
9120 SA=SA+AD: IF SA=3 THEN AD=-1 ELSE IF SA=0 THEN AD=1
9121 ST=ST+1:IF ST=3 THEN ST=0
9122 IF ST=1 THEN YO=1 ELSE YO=0
9140 TT = &H1800+((Y+32)\8)*32
9141 T0 = VPEEK (TT+(X+1)\8)
9142 T1 = VPEEK (TT+(X+8)\8)
9143 T2 = VPEEK (TT+(X+15)\8)
9144 TT = TT + X\8
9150 IF D=0 THEN T3=VPEEK(TT-&H80+2):T4=VPEEK(TT-&H60+2):T5=VPEEK(TT-&H40+2):T6=VPEEK(TT-&H20+2)
9151 IF D=11 THEN T3=VPEEK(TT-&H80):T4=VPEEK(TT-&H60):T5=VPEEK(TT-&H40):T6=VPEEK(TT-&H20)
9180 IF T0<64 AND T1<64 AND T2<64 THEN GS=3:VX=0
9181 IF T3>=64 OR T4>=64 OR T5>=64 OR T6>=64 THEN X=X-VX
9190 RETURN
...
9241 T0 = VPEEK (TT+X\8)
9242 T1 = VPEEK (TT+(X+16)\8)
9243 TT = TT + X\8
9244 IF T0>=64 OR T1>=64 THEN VY=0:Y=Y-VY:Y=((Y+2)\8+1)*8-2:GS=3
...
9300 'GS=3 Falling
9301 VY=VY+1: IF VY>12 THEN VY=12
9302 Y=Y+VY/4:X=X+VX
9340 TT = &H1800+((Y+32)\8)*32
9341 T0 = VPEEK (TT+X\8)
9342 T1 = VPEEK (TT+(X+16)\8)
9343 TT = TT + X\8
9350 IF T0>=64 OR T1>=64 THEN GS=1:VX=0:Y=((Y+32)\8)*8-32
9354 IF VX>0 THEN T3=VPEEK(TT-&H80+2):T4=VPEEK(TT-&H60+2):T5=VPEEK(TT-&H40+2):T6=VPEEK(TT-&H20+2):T7=VPEEK(TT+2)
9355 IF VX<0 THEN T3=VPEEK(TT-&H80):T4=VPEEK(TT-&H60):T5=VPEEK(TT-&H40):T6=VPEEK(TT-&H20):T7=VPEEK(TT)
9381 IF T3>=64 OR T4>=64 OR T5>=64 OR T6>=64 OR T7>=64 THEN X=X-VX
9399 RETURN
