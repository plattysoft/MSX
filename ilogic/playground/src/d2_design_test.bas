...
9105 IF NOT(STRIG(0)) THEN JD=0 ELSE IF JD=0 THEN GS=2:VY=-14:SA=0:ST=4:JD=1:DJ=1:RETURN 'JD: Jump Debouncing, DJ=double jump
9106 X=X+VX
9112 IF S=3 THEN IF D=13 OR SA=4 THEN D=0:AD=1:SA=0:ST=0 ELSE GOTO 9118
9113 IF S=7 THEN IF D=0 OR SA=4 THEN D=13:AD=1:SA=0:ST=0 ELSE GOTO 9118
...
9801 IF T5>=64 AND (T4>=64 OR T6>=64) THEN 9802 ELSE 9809
9802 SA=3
9803 IF VY>7 THEN VY=7 ELSE IF VY<-9 THEN VY=-9
9808 IF NOT(STRIG(0)) THEN JD=0 ELSE IF JD=0 THEN GS=2:VY=-14:VX=-VX:JD=1:IF D=0 THEN D=13 ELSE D=0'JD: Jump Debouncing
9809 RETURN
