...
9001 TIME=0
9002 ' UPDATE
9003 ON GS GOSUB 9100, 9200, 9300
9009 ' DRAW
9010 PUT SPRITE 1,(X,Y+YO),15,D:PUT SPRITE 0,(X,Y+4+YO),4,6+SA+D
9020 PUT SPRITE 2,(X,Y+14),14,1+ST+D
9089 ' END GAME LOOP
9090 IF TIME<6 GOTO 9090 ELSE 9000

9100 'GS=1 Standing
9101 S=STICK(0)
9102 IF STRIG(0) THEN GS=2:VY=-6:VX=0 ELSE GOTO 9112
9103 IF S=3 THEN VX=3
9104 IF S=7 THEN VX=-3
9109 RETURN
9112 IF S=3 THEN X=X+3:IF D=11 OR SA=4 THEN D=0:AD=1:SA=0:ST=0 ELSE GOTO 9118
9113 IF S=7 THEN X=X-3:IF D=0 OR SA=4 THEN D=11:AD=1:SA=0:ST=0 ELSE GOTO 9118
9114 IF S=0 THEN SA=4:ST=3:YO=0' SA=4 marks a resting position
9115 IF S=5 THEN YO=3:ST=4:SA=4
9117 GOTO 9140
9118 SA=SA+AD: IF SA=3 THEN AD=-1 ELSE IF SA=0 THEN AD=1
9119 ST=ST+1:IF ST=3 THEN ST=0
9110 IF ST=1 THEN YO=1 ELSE YO=0
9140 TT = &H1800+((X)\8)+((Y+32)\8)*32
9141 T0 = VPEEK (TT)
9142 T1 = VPEEK (TT+1)
9143 T2 = VPEEK (TT+2)
9144 IF D=0 THEN T3=VPEEK(TT-&H80+2):T4=VPEEK(TT-&H60+2):T5=VPEEK(TT-&H40+2):T6=VPEEK(TT-&H20+2)
9145 IF D=11 THEN T3=VPEEK(TT-&H80):T4=VPEEK(TT-&H60):T5=VPEEK(TT-&H40):T6=VPEEK(TT-&H20)
9146 GOSUB 9900 'VISUAL DEBUG
9180 IF T0<64 AND T1<64 AND T2<64 THEN GS=3
9181 IF T3>=64 OR T4>=64 OR T5>=64 OR T6>=64 THEN IF D=0 THEN X=X-3 ELSE X=X+3
9190 RETURN
...
9211 Y=Y+VY
9212 X=X+VX
9240 TT=&H1800+((X)\8)+((Y+2)\8)*32 ' TODO: Check hit ceiling
9241 T0 = VPEEK (TT)
9242 T1 = VPEEK (TT+1)
9243 T2 = VPEEK (TT+2)
9244 IF T0>=64 OR T1>=64 OR T2>=64 THEN VY=0:Y=Y-VY:Y=((Y+2)\8+1)*8-2:GS=3
9250 TT = &H1800+((X)\8)+((Y+32)\8)*32
9254 IF VX>0 THEN T3=VPEEK(TT-&H80+2):T4=VPEEK(TT-&H60+2):T5=VPEEK(TT-&H40+2):T6=VPEEK(TT-&H20+2):T7=VPEEK (TT+2)
9255 IF VX<0 THEN T3=VPEEK(TT-&H80):T4=VPEEK(TT-&H60):T5=VPEEK(TT-&H40):T6=VPEEK(TT-&H20):T7=VPEEK (TT)
9256 GOSUB 9900 'VISUAL DEBUG

9281 IF T3>=64 OR T4>=64 OR T5>=64 OR T6>=64 OR T7>=64 THEN X=X-VX
9299 RETURN

9300 'GS=3 Falling
9301 VY=VY+1: IF VY>5 THEN VY=5
9302 Y=Y+VY:X=X+VX
9310 TT = &H1800+(X\8)+((Y+32)\8)*32
9311 T0 = VPEEK (TT)
9322 T1 = VPEEK (TT+1)
9323 T2 = VPEEK (TT+2)
9340 IF T0>=64 OR T1>=64 OR T2>=64 THEN GS=1:VX=0:Y=((Y+32)\8)*8-32
9344 IF VX>0 THEN T3=VPEEK(TT-&H80+2):T4=VPEEK(TT-&H60+2):T5=VPEEK(TT-&H40+2):T6=VPEEK(TT-&H20+2)
9345 IF VX<0 THEN T3=VPEEK(TT-&H80):T4=VPEEK(TT-&H60):T5=VPEEK(TT-&H40):T6=VPEEK(TT-&H20)
9341 ' Adjust the rounding to 8
9381 IF T3>=64 OR T4>=64 OR T5>=64 OR T6>=64 THEN X=X-VX
9399 RETURN
