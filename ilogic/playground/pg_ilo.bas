...
239 GOTO 300
270 VX=-4'TH=VPEEK(&H1800+X\8+((Y+8)\8)*32)
...
281 VX=-4'TH=VPEEK(&H1800+X\8+((Y+8)\8)*32)
...
333 IF TU>31 OR TV>31 OR TW>31 THEN PJ%=0:VY%=0:Y=Y-(Y MOD 8) ELSE PJ%=-1:VY%=VY%+1:IF VY%>8 THEN VY%=8
339 GOTO 350
340 TV=VPEEK(&H1800+(X+8)\8+(Y\8)*32)
341 TW=VPEEK(&H1800+(X+2)\8+(Y\8)*32)
342 TU=VPEEK(&H1800+(X+12)\8+(Y\8)*32)
343 IF TU>31 OR TV>31 OR TW>31 THEN VY%=0:Y=Y+8-(Y MOD 8) ELSE VY%=VY%+1:IF VY%>8 THEN VY%=8
350 ' CONVOY BELT BEHAVIOUR 
351 IF TV>=128 AND TV<=132 THEN VX=VX-4':TH=VPEEK(&H1800+(X)\8+((Y+8)\8)*32)
352 IF TV>=133 AND TV<=139 THEN VX=VX-2':TH=VPEEK(&H1800+(X)\8+((Y+8)\8)*32)
353 IF TV>=146 AND TV<=151 THEN VX=VX+2':TH=VPEEK(&H1800+(X+16)\8+((Y+8)\8)*32)
354 IF TV>=152 AND TV<=157 THEN VX=VX+4':TH=VPEEK(&H1800+(X+16)\8+((Y+8)\8)*32)
...
363 TI=VPEEK(&H1800+(X+16)\8+((Y+12)\8)*32)
364 TG=VPEEK(&H1800+(X+16)\8+((Y+2)\8)*32)
