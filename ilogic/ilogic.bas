...
190 PS=1:X=102:Y=120:VY=0
200 ON STICK(0) GOTO 210,220,230,,,,270, 280
201 GOTO 300
210 IF PJ=0 THEN VY=-8:PJ=1
219 GOTO 300
220 IF PJ=0 THEN VY=-8:PJ=1
221 PS=1:IF FN FF(X+16,Y+12)=0 AND FN FF(X+16,Y+8)=0 AND FN FF(X+16,Y+2)=0 THEN X=X+4
229 GOTO 300
230 IF FN FF(X+16,Y+12)=0 AND FN FF(X+16,Y+8)=0 AND FN FF(X+16,Y+2)=0 THEN X=X+4
231 IF PS=1 AND PJ=0 THEN PS=2 ELSE PS=1
239 GOTO 300
270 IF FN FF(X,Y+12)=0 AND FN FF(X,Y+8)=0 AND FN FF(X,Y+2)=0 THEN X=X-4
271 IF PS=3 AND PJ=0 THEN PS=4 ELSE PS=3
279 GOTO 300
280 IF PJ=0 THEN VY=-8:PJ=1
281 PS=3:IF FN FF(X,Y+12)=0 AND FN FF(X,Y+8)=0 AND FN FF(X,Y+2)=0 THEN X=X-4
289 GOTO 300
300 'End of movement loop 
301 'Check for floor tiles at 3 points under the sprite Y+16,(X, X+8, X+15)'
310 Y=Y+VY
320 IF VY<0 GOTO 340
330 IF FN FF(X+2,Y+16)=1 OR FN FF(X+8,Y+16)=1 OR FN FF(X+12,Y+16)=1 THEN PJ=0:VY=0:Y=Y-(Y MOD 8) ELSE VY=VY+1:IF VY>8 THEN VY=8
339 GOTO 390
340 IF FN FF(X+2,Y)=1 OR FN FF(X+8,Y)=1 OR FN FF(X+12,Y)=1 THEN VY=0:Y=Y+8-(Y MOD 8) ELSE PJ=-1:VY=VY+1:IF VY>8 THEN VY=8
