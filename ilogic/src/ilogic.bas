...
9114 SA=4:ST=3:YO=0 'SA=4 marks a resting position, make sure to correct LO Lef Offset
...
9200 'GS=2 Jumping
9210 VY=VY+5: IF VY>=0 THEN GS=3
9211 Y=Y+VY/6
9212 X=X+VX
9213 ST=5:YO=0
9214 SA=3
...
9385 IF T0>=124 OR T1>=124 OR T2>=124 THEN GS=1:JD=1:DJ=0:SA=4:ST=4:YO=4:NK=1:VX=0:Y=((Y+32)/8)*8-32:CMD PLYSOUND 10
...
9449 IF VY>0 AND (T0>=124 OR T1>=124 OR T2>=124) THEN GS=1:JD=1:DJ=0:SA=4:ST=4:YO=4:NK=1:VX=0:Y=((Y+32)/8)*8-32:LO=16
9450 S=STICK(SS)
...
9824 SA=0:JD=1:DJ=0:VY=VY-38:IF VY<-38 THEN VY=-38 ELSE IF VY>-28 THEN VY=-28
9825 S=STICK(SS)
...
10093 T$="@@@DEREZ @"+T$:TX=1:TY=22:GOSUB 10210
10094 GOSUB 8675
10095 T$="@@@TIME @"+T$:TX=21:TY=22:GOSUB 10210
