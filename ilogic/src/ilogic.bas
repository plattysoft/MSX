...
9140 IF STRIG(SS) AND JD=0 THEN GS=2:VY=-14:SA=0:ST=4:JD=1:DJ=1:WT=4:CMD PLYSOUND 8:RETURN 'JD: Jump Debouncing, DJ=double jump
9141 X=X+VX
9142 TT = ((Y+32)/8)*32
9143 T0 = VP (TT+(X+2)/8)
9144 T1 = VP (TT+(X+8)/8)
...
9197 IF X>=238 THEN C=C+1:X=2:GOSUB 8800' Load new room
9198 IF X<=1 THEN C=C-1:X=236:GOSUB 8800' Load new room
9199 RETURN

9200 'GS=2 Jumping
9210 VY=VY+2: IF VY>=0 THEN GS=3:CMD PLYSOUND 9
9211 Y=Y+VY/2
9212 X=X+VX
9213 ST=5
9214 SA=3
...
9297 IF X>=238 THEN C=C+1:X=2:GOSUB 8800' Load new room
9298 IF X<=1 THEN C=C-1:X=236:GOSUB 8800' Load new room
9299 RETURN

9300 'GS=3 Falling
9301 VY=VY+2: IF VY>14 THEN VY=14
9302 Y=Y+VY/2:X=X+VX
9314 SA=0:ST=6
...
9492 IF STRIG(SS) AND JD=0 THEN GS=2:VY=-14:VX=-VX:JD=1:WT=4:CMD PLYSOUND 7:IF D=0 THEN D=14 ELSE D=0'JD: Jump Debouncing
...
9601 IF BT=0 THEN BT=1:TC=1:GOSUB 9610:BT=300:TS=150 ELSE BT=0:GOSUB 9610
9602 CMD PLYSOUND 5
9609 RETURN
...
9651 IF TC=1 THEN TC=0:TS=TS-5 ELSE TC=1:IF TS>50 THEN TS=TS-45 ELSE TS=TS-10
...
9823 IF JD=0 AND STRIG(SS) THEN GS=2:CMD PLYSOUND 8:SA=0:ST=4:JD=1:DJ=0:VY=VY-14:IF VY<-14 THEN VY=-14 ELSE IF VY>-6 THEN VY=-6
9829 RETURN
...
9913 IF EX(EI) MOD 8=0 THEN EL=VP(((EY(EI)+9)\8)*32+(EX(EI)+8+2*EV(EI))\8):IF EL=138 OR EL=140 THEN EV(EI)=-EV(EI)
9919 RETURN

9920 'fun Update enemy type 2: Horizontal, upper
9921 EX(EI)=EX(EI)+EV(EI)
9923 IF EX(EI) MOD 8=0 THEN EL=VP(((EY(EI)+8)\8)*32+(EX(EI)+8+2*EV(EI))\8):IF EL=170 OR EL=172 THEN EV(EI)=-EV(EI)
9929 RETURN

9930 'fun Update enemy type 3: Vertical, right wall
9931 EY(EI)=EY(EI)+EV(EI)
9933 IF EY(EI) MOD 8=6 THEN EL=VP(((EY(EI)+8+3*EV(EI))\8)*32+(EX(EI)+8)\8):IF EL=142 OR EL=174 THEN EY(EI)=EY(EI)-EV(EI):EV(EI)=-EV(EI)
9939 RETURN

9940 'fun Update enemy type 4: Vertical, left wall
9941 EY(EI)=EY(EI)+EV(EI)
9943 IF EY(EI) MOD 8=6 THEN EL=VP(((EY(EI)+8+3*EV(EI))\8)*32+(EX(EI))\8):IF EL=141 OR EL=173 THEN EY(EI)=EY(EI)-EV(EI):EV(EI)=-EV(EI)
9949 RETURN

10000 'fun ending
10001 CMD RESTORE 43:RESTORE 16
