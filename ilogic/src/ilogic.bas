...
5160 ' --- Slot 5
5170 ' color 14
5180 DATA 00,07,1F,3F,7F,7F,FE,FC,F8,FC,FE,7F,7F,3F,1F,07
5190 DATA 00,00,00,00,00,00,00,00,00,00,00,00,00,00,00,00
5200 '
5210 ' --- Slot 6
5220 ' color 14
5230 DATA 00,07,1F,3F,7F,7F,FE,F8,FC,FC,FD,7F,7F,3F,1F,07
5240 DATA 00,00,00,00,00,00,00,00,00,00,00,00,00,00,00,00
5250 '
5260 ' --- Slot 7
5270 ' color 14
5280 DATA 00,07,1F,3F,7F,7F,FD,FC,FC,F8,FF,7F,7F,3F,1F,07
5290 DATA 00,00,00,00,00,00,00,00,00,00,00,00,00,00,00,00
...
8102 Y=20
8103 DIM EX(4),EY(4),EV(4),ET(4),ES(4),EW(4)
...
8822  IF TT=160 THEN EC=EC+1:ET(EC)=2:EV(EC)=1:EX(EC)=(I MOD 32)*8:EY(EC)=(I\32)*8-2:VPOKE &H1800+I,0:VPOKE &H1800+I+1,0
8830 NEXT I
...
9905 FOR EI=1 TO EC
9906  ON ET(EI) GOSUB 9910,9920,9930,9940
9907 NEXT I
9909 RETURN

9910 EX(EI)=EX(EI)+EV(EI)
9911 EW(EI)=EW(EI)+1:IF EW(EI)=3 THEN EW(EI)=0:ES(EI)=ES(EI)+EV(EI):IF ES(EI)=3 THEN ES(EI)=0 ELSE IF ES(EI)=-1 THEN ES(EI)=2
9912 IF EX(EI) MOD 8=0 THEN EL=VPEEK(&H1800+((EY(EI)+9)\8)*32+(EX(EI)+8+4*EV(EI))\8):IF EL=75 OR EL=77 THEN EV(EI)=-EV(EI)
9919 RETURN

9920 EX(EI)=EX(EI)+EV(EI)
9921 EW(EI)=EW(EI)+1:IF EW(EI)=3 THEN EW(EI)=0:ES(EI)=ES(EI)+EV(EI):IF ES(EI)=3 THEN ES(EI)=0 ELSE IF ES(EI)=-1 THEN ES(EI)=2
9922 IF EX(EI) MOD 8=0 THEN EL=VPEEK(&H1800+((EY(EI))\8)*32+(EX(EI)+8+4*EV(EI))\8):IF EL=107 OR EL=109 THEN EV(EI)=-EV(EI)
9929 RETURN

9930 'VERTICAL MOVEMENT TODO
9939 RETURN

9940 'VERTICAL MOVEMENT TODO
9949 RETURN
