...
111 'F is <35, then in changes of 6, 3 mice get a +.Rank is 0 for F, 1 for E... 3 for C (B is minimum for certified pilot) ...6 for S
111 RK = (TM - 36) \ 6

120 IF TM=72 THEN VPOKE &H1800+9*32+19, 114 ELSE VPOKE &H1800+9*32+19, 101-RK
121 IF TM MOD 6 >= 3 THEN VPOKE &H1800+9*32+20, 43
190 TD = TM \ 10
191 TU = TM MOD 10
200 IF TD > 0 THEN VPOKE &H1800+12*32+19, 33+TD: VPOKE &H1800+12*32+20, 33+TU ELSE VPOKE &H1800+12*32+19, 33+TU

300 'CERTIFIED PILOT STAMP 
300 IF TM<60 THEN BLOAD "bad_end.scr" ELSE IF TM=72 THEN BLOAD "best_end.scr",S:PLAY "S9M3050L4O1B" ELSE BLOAD "good_end.scr",S:PLAY "S9M3050L4O1B" 
