...
2921 M=VPEEK(15)-1
2922 IF M>LL(L-1) THEN LL(L-1)=M
...
2946 IF STRIG(SS) THEN IF Y=32 THEN CMD WRTVRAM 25,0:GOTO 4100 ELSE IF Y=0 THEN L=L+1: GOTO 190 ELSE GOTO 190
2950 GOTO 2940
...
2972 IF STRIG(SS) THEN IF Y=16 THEN CMD WRTVRAM 25,0:GOTO 4100 ELSE GOTO 190
2980 GOTO 2960

3500 ' START.BAS
3501 CMD WRTSCR 24
3502 STRIG(0) ON: STRIG(1) ON
3503 ON STRIG GOSUB 3990, 3990
...
3720    VP = VPEEK(&HCC0+J+K)
3721    VPOKE &HCBF+J+K, VP
3722    VP = VPEEK(&H2CC0+J+K)
3723    VPOKE &H2CBF+J+K, VP
3747   NEXT K
3748  NEXT J
3750  FOR K=0 TO &H300 STEP &H100
3751   VP=VPEEK(&H4E0+I+K)
3752   VPOKE &HCDF+K, VP
3753   VP=VPEEK(&H2CE0+I+K)
3754   VPOKE &H2CDF+K, VP
3796  NEXT K
...
4502 SL=(X-8)\40+(Y-8)*6\40+1
4503 IF SL>1 THEN IF LL(SL-2)=-1 THEN GOTO 4200
...
5112 FOR I=0 TO 23 STEP 2
5113  VPOKE I/2+2, (LL(I)+1)*16+(LL(I+1)+1)
5114 NEXT I
5120 GOTO 4100
...
6120 IF TM=72 THEN VPOKE &H1800+9*32+19, 114 ELSE VPOKE &H1800+9*32+19, 101-RK
6121 IF TM MOD 6 >= 3 THEN VPOKE &H1800+9*32+20, 43
6190 TD = TM \ 10
6191 TU = TM MOD 10
6200 IF TD > 0 THEN VPOKE &H1800+12*32+19, 33+TD: VPOKE &H1800+12*32+20, 33+TU ELSE VPOKE &H1800+12*32+19, 33+TU

6300 'CERTIFIED PILOT STAMP
6301 'IF TM<60 THEN BLOAD "bad_end.plet5",S ELSE IF TM=72 THEN BLOAD "best_end.plet5",S:PLAY "S9M3050L4O1B" ELSE BLOAD "good_end.plet5",S:PLAY "S9M3050L4O1B"

6420 IF INKEY$<>"" THEN 6420
6421 IF INKEY$="" AND NOT STRIG(1) THEN 6421

6430 'BLOAD "loading.sc2",S:GOSUB 3000:GOTO 4000
