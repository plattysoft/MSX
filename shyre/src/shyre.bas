...
271 GOSUB 900
280 X=88:Y=160:GS=1:MD=1:ES=1:RR=4:RC=2:RH=0
...
419 GOTO 490
420 IF VPEEK(PT-96)<22 AND VPEEK(PT-95)<22 THEN GS=3:MS=8:PUT SPRITE 2,(X,Y-17),9,0:PP=PT-64:PF=PP-32:CS=9
429 GOTO 490
...
439 GOTO 490
440 IF VPEEK(PT+4)<22 AND VPEEK(PT+36)<22 THEN GS=3:MS=8:PUT SPRITE 2,(X+16,Y-1),9,0:PP=PT+2:PF=PP+1:CS=15
449 GOTO 490
...
459 GOTO 490
460 IF VPEEK(PT+128)<22 AND VPEEK(PT+129)<22 THEN GS=3:MS=8:PUT SPRITE 2,(X,Y+15),9,0:PP=PT+64:PF=PP+32:CS=21
469 GOTO 490
...
479 GOTO 490
480 IF VPEEK(PT-3)<22 AND VPEEK(PT+29)<22 THEN GS=3:MS=8:PUT SPRITE 2,(X-16,Y-1),9,0:PP=PT-2:PF=PP-1:CS=27
489 GOTO 490
...
750 IF RH=0 THEN CR=1+RR*5+RC ELSE CR=26+(RR-1)*3+RC-1
751 CMD WRTSCR CR
...
790 ' Redraw (to be reused when loading rooms and when picking up items)
