...
490 ' Other checks
495 IF T1<64 AND T2<64 THEN GS=2:MS=4:GOSUB 500
...
582 T1=VPEEK(PT)
583 IF T1=28 THEN RH=RH+1:GOSUB 750
584 IF T1=30 THEN RH=RH-1:GOSUB 750
585 IF T1=22 THEN I1=1:GOSUB 780
586 IF T1=24 THEN I2=1:GOSUB 780
587 IF T1=26 THEN MP=1:GOSUB 780
599 RETURN
...
652 GS=1
653 IF TL(PF-&H1800)>=13 AND TL(PF-&H1800)<=16 AND TL(PF+1-&H1800)>=13 AND TL(PF+1-&H1800)<=16 AND TL(PF+32-&H1800)>=13 AND TL(PF+32-&H1800)<=16 AND TL(PF+33-&H1800)>=13 AND TL(PF+33-&H1800)<=16 GOSUB 675 'Icy floor
654 VPOKE PF,130:VPOKE PF+1,131:VPOKE PF+32,162:VPOKE PF+33,163:PUT SPRITE 2,(0,0),0,0
655 IF TL(PF-&H1800)=19 THEN GOSUB 660 'Open door
656 IF TL(PP-&H1800)=19 THEN GOSUB 670 'Close door
659 RETURN

660 FOR I=0 TO 767
661   IF TL(I)=81 THEN VPOKE &H1800+I,17:VPOKE &H1800+I+1,18:VPOKE &H1800+I+32,49:VPOKE &H1800+I+33,50:RETURN
662 NEXT I
669 RETURN

670 FOR I=0 TO 767
671   IF TL(I)=81 THEN VPOKE &H1800+I,81:VPOKE &H1800+I+1,82:VPOKE &H1800+I+32,113:VPOKE &H1800+I+33,114:RETURN
672 NEXT I
674 RETURN
...
686 IF T1>=13 AND T1<=16 AND T2>=13 AND T2<=16 THEN PP=PT:GOTO 690
689 RETURN

690 FOR I=1 TO 4'FIXE it slides too much (out of sync)
...
791 IF MP=1 THEN VPOKE &H1860+27, 26:VPOKE &H1860+28, 27: VPOKE &H1860+59, 58:VPOKE &H1860+60, 59
792 IF I1=1 THEN VPOKE &H1860+25, 22:VPOKE &H1860+26, 23: VPOKE &H1860+57, 54:VPOKE &H1860+58, 55
793 IF I2=1 THEN VPOKE &H1860+29, 24:VPOKE &H1860+30, 25: VPOKE &H1860+61, 56:VPOKE &H1860+62, 57
799 RETURN
...
910 FOR I=0 TO 767
920   T=VPEEK(&H1800+I)
930   IF T<130 THEN TL(I)=T ELSE TL(I)=T-128
940   IF T>=7 AND T<=10 THEN EY=(I/32)*8:EX=((I MOD 32)-1)*8:ED=T-6:EM=8
941   IF MP=1 AND (T=26 OR T=27 OR T=58 OR T=59) THEN VPOKE &H1800+I,0
942   IF I1=1 AND (T=22 OR T=23 OR T=54 OR T=55) THEN VPOKE &H1800+I,0
943   IF I2=1 AND (T=24 OR T=25 OR T=56 OR T=57) THEN VPOKE &H1800+I,0
950 NEXT I
960 RETURN
