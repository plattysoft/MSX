...
60 ' --- Slot 1
70 ' color 1
80 DATA 00,4D,4D,00,26,26,26,26,4D,4D,00,26,26,26,4D,4D
90 DATA 00,FE,FE,00,FC,FC,FC,FC,FE,FE,00,FC,FC,FC,FE,FE
100 ' 
110 ' --- Slot 2
120 ' color 1
...
413 IF T1=226 THEN PS=1:PC=6:GOTO 420
414 IF T1=9 AND T2=9 THEN RR=RR-1:Y=176: GOSUB 750
415 'IF T1=113 THEN PT=PT-64:GOSUB 760
416 IF T1=178 AND I1=1 THEN PT=PT-64:NT=82:GOSUB 760
419 GOTO 490
420 IF VPEEK(PT-96)<64 AND VPEEK(PT-95)<64 THEN GS=3:MS=8:PUT SPRITE 2,(X,Y-17),PC,PS:PP=PT-64:PF=PP-32:CS=9' This should exit via another route, not 490
429 GOTO 490

430 MD=2:BS=15
431 T1=VPEEK(PT+2):T2=VPEEK(PT+34)
432 IF T1=192 THEN PS=0:PC=9:GOTO 440
433 IF T1=194 THEN PS=1:PC=6:GOTO 440
434 IF T1=8 AND T2=8 THEN RC=RC+1:X=0: GOSUB 750
435 'IF T1=81 THEN PT=PT+2:GOSUB 760
436 IF T1=146 AND I1=1 THEN PT=PT+2:NT=82:GOSUB 760
439 GOTO 490
440 IF VPEEK(PT+4)<64 AND VPEEK(PT+36)<64 THEN GS=3:MS=8:PUT SPRITE 2,(X+16,Y-1),PC,PS:PP=PT+2:PF=PP+1:CS=15' This should exit via another route, not 490
449 GOTO 490

450 MD=3:BS=21
451 T1=VPEEK(PT+64):T2=VPEEK(PT+65)
452 IF T1=192 THEN PS=0:PC=9:GOTO 460
453 IF T1=194 THEN PS=1:PC=6:GOTO 460
454 IF T1=7 AND T2=7 THEN RR=RR+1:Y=0: GOSUB 750
455 'IF T1=81 THEN PT=PT+64:GOSUB 760
456 IF T1=146 AND I1=1 THEN PT=PT+64:NT=82:GOSUB 760
459 GOTO 490
460 IF VPEEK(PT+128)<64 AND VPEEK(PT+129)<64 THEN GS=3:MS=8:PUT SPRITE 2,(X,Y+15),PC,PS:PP=PT+64:PF=PP+32:CS=21' This should exit via another route, not 490
469 GOTO 490

470 MD=4:BS=27
471 T1=VPEEK(PT-1):T2=VPEEK(PT+31)
472 IF T1=193 THEN PS=0:PC=9:GOTO 480
473 IF T1=195 THEN PS=1:PC=6:GOTO 480
474 IF T1=10 AND T2=10 THEN RC=RC-1:X=176: GOSUB 750
475 'IF T1=147 THEN PT=PT-2:GOSUB 760
476 IF T1=147 AND I1=1 THEN PT=PT-2:NT=82:GOSUB 760
479 GOTO 490
480 IF VPEEK(PT-3)<64 AND VPEEK(PT+29)<64 THEN GS=3:MS=8:PUT SPRITE 2,(X-16,Y-1),PC,PS:PP=PT-2:PF=PP-1:CS=27' This should exit via another route, not 490
489 GOTO 490
...
655 IF TL(PF-&H1800)=18 THEN CT=144:NT=16:GOSUB 660 'Open magenta door
656 IF TL(PP-&H1800)=18 THEN CT=16:NT=144:GOSUB 660 'Close magenta door
657 IF TL(PF-&H1800)=22 THEN CT=148:NT=20:GOSUB 660 'Open cyan door
658 IF TL(PP-&H1800)=22 THEN CT=20:NT=148:GOSUB 660 'Close cyan door
659 RETURN

660 FOR I=0 TO 767
661   IF TL(I)=CT THEN TL(I)=NT:TL(I+1)=NT+1:TL(I+32)=NT+32:TL(I+33)=NT+33:PT=&H1800+I:GOSUB 760:RETURN
662 NEXT I
663 RETURN
...
761 VPOKE PT,NT:VPOKE PT+1,NT+1:VPOKE PT+32,NT+32:VPOKE PT+33,NT+33
769 RETURN
...
794 IF I4=1 THEN PT=&H18C0+27:NT=92:GOSUB 760
799 RETURN


800 'ENEMY SECTION: Check for turns
...
941   IF MP=1 AND T=90 THEN VPOKE &H1800+I,0:VPOKE &H1800+I+1,0:VPOKE &H1800+I+32,0:VPOKE &H1800+I+33,0
942   IF I1=1 AND T=86 THEN VPOKE &H1800+I,0:VPOKE &H1800+I+1,0:VPOKE &H1800+I+32,0:VPOKE &H1800+I+33,0
943   IF I2=1 AND T=88 THEN VPOKE &H1800+I,0:VPOKE &H1800+I+1,0:VPOKE &H1800+I+32,0:VPOKE &H1800+I+33,0
944   IF I4=1 AND T=92 THEN VPOKE &H1800+I,0:VPOKE &H1800+I+1,0:VPOKE &H1800+I+32,0:VPOKE &H1800+I+33,0
950 NEXT I
960 RETURN
