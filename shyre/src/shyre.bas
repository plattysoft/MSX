...
280 X=88:Y=192:GS=1:MD=1:CS=19:WS=0
281 IX=X:IY=144:ID=MD:IG=GS:IM=0:IS=CS:IB=0 'Initial state for the first room

289 BS=1*6-3
290 Y=Y-2:TIME=0
291 WS=(WS+1) MOD 8:WT=WS\2:IF WT=2 OR WT=0 THEN CS=4 ELSE IF WT=1 THEN CS=0 ELSE CS=2
292 IF WS=1 THEN CMD PLYSOUND 6
293 CS=BS+CS
294 PUT SPRITE 0,(X,Y),5,CS:PUT SPRITE 1,(X,Y),15,CS+1
295 IF TIME<2 THEN 295 ELSE IF Y>144 THEN 290 ELSE CS=19:WS=0
...
3030 Y=Y-2:TIME=0
3033 WS=(WS+1) MOD 8:WT=WS\2:IF WT=2 OR WT=0 THEN CS=4 ELSE IF WT=1 THEN CS=0:CMD PLYSOUND 6 ELSE CS=2
3034 IF WS=1 THEN CMD PLYSOUND 6
3035 CS=BS+CS
3038 PUT SPRITE 1,(X,Y),,CS:PUT SPRITE 2,(X,Y),,CS+1
3039 IF TIME<2 THEN 3039 ELSE IF Y>100 THEN 3030

3040 CMD PLYSOUND 3:PT=&H19AC:NT=0:GOSUB 765:PT=&H1840+26:NT=82:GOSUB 765'remove the item, mark it within items
3041 PUT SPRITE 1,,,19:PUT SPRITE 2,,,20:PUT SPRITE 3,,,2
3044 IF TIME<100 THEN 3044

3180 TIME=0:I=0:CMD PLYSOUND 12
3181 PUT SPRITE 0,(X,Y),7,I+43
3182 I=I+1
3183 IF TIME<4 THEN GOTO 3183 ELSE IF I<4 THEN TIME=0:GOTO 3181
3184 PUT SPRITE 2,,0:PUT SPRITE 1,,0
...
3301 CMD WRTSCR 0
3313 T$="ELECTRIC STORM INCOMING[#MUST TAKE SHELTER[":TY=7:GOSUB 1290
3314 T$="COMMS ARE JAMMED[#NEED TO FIND A WAY UP[":TY=12:GOSUB 1290
