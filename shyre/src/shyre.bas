...
280 X=88:Y=144:GS=1:MD=1:CS=26:WS=0
...
3000 'intro sequence
3001 IF STRIG(0) OR STRIG(1) GOTO 3001 'Debouncing space press
3002 STRIG(0) ON: STRIG(1) ON
3003 ON STRIG GOSUB 4000, 4000
3010 CMD WRTSCR 0 ' This should be a screen of the ship, with items on the right
3011 CMD WRTCHR 39 ' Load tileset for gameplay
3012 CMD WRTCLR 40
3020 T$="IT IS DANGEROUS TO GO ALONE[#      I\LL TAKE THIS["
3021 TX=2:TY=7:GOSUB 1900
3022 PT=&H1800+32*13+14:VPOKE PT,82:VPOKE PT+1,83:VPOKE PT+32,114:VPOKE PT+33,115

3029 X=112:Y=170
3030 Y=Y-2
3033 WS=(WS+1) MOD 8:WT=WS\2:IF WT=2 OR WT=0 THEN CS=6 ELSE IF WT=1 THEN CS=0 ELSE CS=3
3034 CS=2+CS
3035 IF Y=100 THEN 3800
3038 PUT SPRITE 1,(X,Y),5,CS+1:PUT SPRITE 2,(X,Y),15,CS+2:PUT SPRITE 0,(X,Y),1,CS
3039 IF TIME<3 THEN 3039 ELSE TIME=0: GOTO 3030

3800 IF NOT STRIG(SS) THEN 3020
3900 CMD WRTSCR 0
3999 RETURN 250
