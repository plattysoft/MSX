...
281 IX=X:IY=Y:ID=MD:IG=GS:IM=0:IS=CS:IB=0 'Initial state for the first room

300 TIME=0' BEGIN GAME LOOP
301 ON GS GOSUB 400,500,600,700,720

389 PUT SPRITE 0,(X,Y),5,CS:PUT SPRITE 1,(X,Y),15,CS+1:PUT SPRITE 2,(X,Y),1,2
...
404 IF SR=10 THEN SR=0:GOSUB 1100
409 RETURN
...
414 IF T1=60 AND T2=60 THEN RR=RR-1:Y=176:GS=2:MS=4:BS=MD*6-3:GOSUB 740
416 IF T1=178 THEN IF I1=1 THEN CMD PLYSOUND 13:GS=5:MS=4:PT=PT-64:NT=10:RETURN ELSE GOSUB 1410
417 IF T1=182 THEN IF I5=1 THEN CMD PLYSOUND 14:GS=5:MS=4:PT=PT-64:NT=70:RETURN ELSE GOSUB 1414
419 GOTO 490
420 IF VPEEK(PT-96)<60 AND VPEEK(PT-95)<60 THEN GS=3:MS=8:CMD PLYSOUND 7:PUT SPRITE 3,(X,Y-17),PC,PS:PP=PT-64:PF=PP-32 ELSE GS=4:MS=8
429 GOTO 490
...
434 IF T1=61 AND T2=61 THEN RC=RC+1:X=0:GS=2:MS=4:BS=MD*6-3:GOSUB 740
436 IF T1=146 THEN IF I1=1 THEN CMD PLYSOUND 13:GS=5:MS=4:PT=PT+2:NT=10:RETURN ELSE GOSUB 1410
437 IF T1=150 THEN IF I5=1 THEN CMD PLYSOUND 14:GS=5:MS=4:PT=PT+2:NT=70:RETURN ELSE GOSUB 1414
439 GOTO 490
440 IF VPEEK(PT+4)<60 AND VPEEK(PT+36)<60 THEN GS=3:MS=8:CMD PLYSOUND 7:PUT SPRITE 3,(X+16,Y-1),PC,PS:PP=PT+2:PF=PP+1 ELSE GS=4:MS=8
449 GOTO 490
...
454 IF T1=62 AND T2=62 THEN RR=RR+1:Y=0:GS=2:MS=4:BS=MD*6-3:GOSUB 740
456 IF T1=146 THEN IF I1=1 THEN CMD PLYSOUND 13:GS=5:MS=4:PT=PT+64:NT=10:RETURN ELSE GOSUB 1410
457 IF T1=150 THEN IF I5=1 THEN CMD PLYSOUND 14:GS=5:MS=4:PT=PT+64:NT=70:RETURN ELSE GOSUB 1414
459 GOTO 490
460 IF VPEEK(PT+128)<60 AND VPEEK(PT+129)<60 THEN GS=3:MS=8:CMD PLYSOUND 7:PUT SPRITE 3,(X,Y+15),PC,PS:PP=PT+64:PF=PP+32 ELSE GS=4:MS=8
469 GOTO 490
...
474 IF T1=63 AND T2=63 THEN RC=RC-1:X=176:GS=2:MS=4:BS=MD*6-3:GOSUB 740
476 IF T1=147 THEN IF I1=1 THEN CMD PLYSOUND 13:GS=5:MS=4:PT=PT-2:NT=10:RETURN ELSE GOSUB 1410
477 IF T1=151 THEN IF I5=1 THEN CMD PLYSOUND 14:GS=5:MS=4:PT=PT-2:NT=70:RETURN ELSE GOSUB 1414
479 GOTO 490
480 IF VPEEK(PT-3)<60 AND VPEEK(PT+29)<60 THEN GS=3:MS=8:CMD PLYSOUND 7:PUT SPRITE 3,(X-16,Y-1),PC,PS:PP=PT-2:PF=PP-1 ELSE GS=4:MS=8
489 GOTO 490

490 ' Other checks
491 TT=VPEEK(PT)
495 IF T1<128 AND T2<128 THEN GS=2:MS=4:BS=MD*6-3:CMD PLYSOUND 6:GOSUB 500
496 IF GS=3 THEN BS=23+MD*4:VPOKE PP,TL(PP-&H1800):VPOKE PP+1,TL(PP+1-&H1800):VPOKE PP+32,TL(PP+32-&H1800):VPOKE PP+33,TL(PP+33-&H1800)
499 RETURN
...
560 WS=(WS+1) MOD 8:WT=WS\2:IF WT=2 OR WT=0 THEN CS=4 ELSE IF WT=1 THEN CS=0 ELSE CS=2
561 CS=BS+CS
569 RETURN
...
611 Y=Y-1:PUT SPRITE 3,(X,Y-17):GOTO 620
612 X=X+1:PUT SPRITE 3,(X+16,Y-1):GOTO 620
613 Y=Y+1:PUT SPRITE 3,(X,Y+15):GOTO 620
614 X=X-1:PUT SPRITE 3,(X-16,Y-1):GOTO 620

620 MS=MS-1
621 IF MS>6 OR MS<=2 THEN CS=BS ELSE CS=BS+2' Implement common for all directions
...
710 MS=MS-1
711 IF MS>6 OR MS<=2 THEN CS=23+MD*4 ELSE CS=25+MD*4' Implement common for all directions
...
1101 I=0:CMD PLYSOUND 12
1102 PUT SPRITE 0,(X,Y),13,I+43
1103 I=I+1
1104 IF TIME<4 THEN GOTO 1104 ELSE IF I<4 THEN TIME=0:GOTO 1102
1105 PUT SPRITE 2,,0:PUT SPRITE 3,,0:PUT SPRITE 1,,0
1106 PUT SPRITE 0,,,I+43
1107 I=I-1
1109 IF TIME<4 THEN GOTO 1109 ELSE IF I>=0 THEN TIME=0:GOTO 1106
1110 PUT SPRITE 0,(0,0),0
1111 X=IX:Y=IY:GS=IG:MD=ID:MS=IM:CS=IS:BS=IB 'Remember starting position on a room
1120 GOSUB 750
1130 RETURN 300

1200 ' Ending
1201 I=0:CMD PLYSOUND 12
1202 PUT SPRITE 0,(X,Y),7,I+43
1203 I=I+1
1204 IF TIME<4 THEN GOTO 1204 ELSE IF I<4 THEN TIME=0:GOTO 1202
1205 PUT SPRITE 2,,0:PUT SPRITE 3,,0:PUT SPRITE 1,,0
1206 PUT SPRITE 0,,,I+43
1207 I=I-1
1209 IF TIME<4 THEN GOTO 1209 ELSE IF I>=0 THEN TIME=0:GOTO 1206
1210 PUT SPRITE 0,(0,0),0
1220 'TODO Load the spaceship, last remarks and then THE END
1250 IF TIME<50 THEN 1250
1290 CMD WRTSCR 0' THE END Screen
1291 CMD WRTCHR 41
1292 CMD WRTCLR 42
1293 CMD WRTSCR 44
1294 'Wait for press space
...
1308 PUT SPRITE 0,,0,0:PUT SPRITE 1,,0::PUT SPRITE 2,,0
...
3000 GOTO 3900 'intro sequence, temporary skip
3001 CMD WRTSCR 0
3002 CMD WRTCHR 39 ' Load tileset for gameplay
3003 CMD WRTCLR 40
3004 IF STRIG(0) OR STRIG(1) GOTO 3004 'Debouncing space press
3005 STRIG(0) ON: STRIG(1) ON
3006 ON STRIG GOSUB 4000, 4000

3010 CMD WRTSCR 45 ' Pick up the datapad
3020 T$="IT IS DANGEROUS TO GO#ALONE[ I\LL TAKE THIS["
3021 TX=2:TY=7:GOSUB 1900
3022 PT=&H19AC:NT=82:GOSUB 765

3023 X=96:Y=170:BS=3*6-3
3024 TIME=0:PUT SPRITE 1,(X,Y),5,BS:PUT SPRITE 2,(X,Y),15,BS+1:PUT SPRITE 3,(X,Y),1,2
3025 IF TIME<25 THEN 3025
3026 BS=1*6-3

3030 Y=Y-2
3033 WS=(WS+1) MOD 8:WT=WS\2:IF WT=2 OR WT=0 THEN CS=4 ELSE IF WT=1 THEN CS=0 ELSE CS=2
3034 CS=BS+CS
3038 PUT SPRITE 1,(X,Y),,CS:PUT SPRITE 2,(X,Y),,CS+1
3039 IF TIME<3 THEN 3039 ELSE IF Y>100 THEN TIME=0:GOTO 3030

3040 PT=&H19AC:NT=0:GOSUB 765:PT=&H1840+26:NT=82:GOSUB 765'remove the item, mark it within items
3041 PUT SPRITE 1,,,CS:PUT SPRITE 2,,,CS+1:PUT SPRITE 3,,,2

3100 I=0 'talk with the cat and teleport
3101 CS=47+(I MOD 4):I=I+1
3108 PUT SPRITE 2,(50,170),14,CS
3109 IF TIME<10 THEN 3109 ELSE IF I<16 THEN TIME=0:GOTO 3101 ELSE I=0
3110 CS=51+(I MOD 4):I=I+1
3111 PUT SPRITE 2,,,CS
3112 IF TIME<10 THEN 3112 ELSE IF I<4 THEN TIME=0:GOTO 3110
3180 I=0
3181 PUT SPRITE 0,(X,Y),7,I+43
3182 I=I+1
3183 IF TIME<4 THEN GOTO 3180 ELSE IF I<4 THEN TIME=0:GOTO 3181
3184 PUT SPRITE 3,,0:PUT SPRITE 1,,0
3190 PUT SPRITE 0,,,I+43
3191 I=I-1
3199 IF TIME<4 THEN GOTO 3199 ELSE IF I>=0 THEN TIME=0:GOTO 3190

3300 'Storm, take shelter TODO

3800 IF NOT STRIG(SS) THEN 3800
3900 CMD WRTSCR 0
3999 RETURN 250

4000 STRIG(0) OFF: STRIG(1) OFF: RETURN 3900
