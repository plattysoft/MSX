...
100 'TODO: READ S and TR From a file
110 S=0:TR=0
...
331 IF VPEEK(&H1800+(X+2)\8+((Y+16)\8)*32)>0 OR TV>0 OR VPEEK(&H1800+(X+12)\8+((Y+16)\8)*32)>0 THEN VY%=0:Y=Y-(Y MOD 8):IF PJ<>0 THEN PJ%=0:SOUND 0,0:SOUND 1,0: SOUND 13,0: GOTO 350 ELSE GOTO 350
332 VY%=VY%+1:IF VY%>8 THEN VY%=8
333 IF PJ%=0 THEN PJ%=-1:SOUND 1,&H7:SOUND 0,&HF2:SOUND 12,20:SOUND 13,4
339 GOTO 350
...
532 IF PL=-1 THEN R=3:C=9:GOTO 700
533 'Update lifes UI
533 VPOKE &H1AC6,48+PL
534 SOUND 1,&HD:SOUND 0,&H5D:SOUND 12,18:SOUND 13,13
535 K=11
536 TIME=0
537 K=K+1: IF K=19 THEN 199 ELSE IF K=16 THEN SOUND 13,0
