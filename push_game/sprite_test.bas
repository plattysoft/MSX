...
275   IF T>=7 AND T<=10 THEN EY=(I/32)*8:EX=((I MOD 32)-1)*8:ED=T-6:EM=8:ES=1
276 NEXT I
...
530 Y=Y+1:GOTO 550
540 X=X-1:GOTO 550
550 MS=MS-1:IF MS=0 THEN GOTO 560 ELSE GOTO 599

560 GS=1
561 PT=&H1800+X\8+(Y\8)*32
562 PI=VPEEK(PT)
570 IF PI=17 OR PI=19 THEN VPOKE PT,2:VPOKE PT+1,3:VPOKE PT+32,34:VPOKE PT+33,35 ELSE GOTO 599
571 VPOKE &H1800+94,PI:VPOKE &H1800+94+1,PI+1:VPOKE &H1800+94+32,PI+32:VPOKE &H1800+94+33,PI+33
572 IF PI<>17 THEN GOTO 599
575 FOR I=0 TO 767
580   IF TL(I)=28 THEN VPOKE &H1800+I,30:VPOKE &H1801+I,31:VPOKE &H1820+I,62:VPOKE &H1821+I,63
581 NEXT I
599 RETURN
...
700 ' ENEMY MOVES
700 ON ES GOSUB 709, 800
701 PUT SPRITE 2,(EX,EY-1),4,ED+4
702 RETURN
...
810 IF E0=36 THEN ED=2:RETURN
811 IF E0=37 THEN ED=4:RETURN
815 T1=ET+64:T2=T1+1
819 GOTO 850

820 IF E0=5 THEN ED=1:RETURN
821 IF E0=37 THEN ED=3:RETURN
825 T1=ET+2:T2=T1+32
829 GOTO 850

830 IF E0=4 THEN ED=2:RETURN
831 IF E0=5 THEN ED=4:RETURN
835 T1=ET-32:T2=T1+1
839 GOTO 850

840 IF E0=4 THEN ED=1:RETURN
841 IF E0=36 THEN ED=3:RETURN
845 T1=ET-1:T2=T1+32
849 GOTO 850
